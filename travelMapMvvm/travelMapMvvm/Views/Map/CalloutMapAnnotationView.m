...
- (id) initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
	if (self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier]) {
		self.contentHeight = 60.0;
        self.offsetFromParent = CGPointMake(8, -14); //this works for MKPinAnnotationView
        self.offsetFromParent = CGPointMake(8, -7); //this works for MKAnnotationView
		self.enabled = NO;
        ...
        [self addConstraint:[NSLayoutConstraint constraintWithItem:_contentView attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeLeading multiplier:1 constant:10]];
        [self addConstraint:[NSLayoutConstraint constraintWithItem:_contentView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1 constant:3]];
        [_contentView addConstraint:[NSLayoutConstraint constraintWithItem:_contentView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1 constant:self.contentHeight-3]];
        [_contentView addConstraint:[NSLayoutConstraint constraintWithItem:_contentView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1 constant:[UIScreen mainScreen].bounds.size.width - 20]];
	}
	return self;
}
...
- (void)adjustMapRegionIfNeeded {
	...
	//Latitude
	CGPoint mapViewOriginRelativeToParent = [self.mapView convertPoint:CGPointZero toView:self.parentAnnotationView];
	...
	if (xPixelShift || yPixelShift) {
        ...
        [self changePositionWithXPixelShift:xPixelShift yPixelShift:yPixelShift];
	}
}

- (void)changePositionWithXPixelShift:(CGFloat) xPixelShift yPixelShift:(CGFloat)yPixelShift {
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([self relativeParentXPosition] < 38) {
        ...
        xPixelShift = (self.mapView.frame.size.width - 38) - [self relativeParentXPosition];
    }
